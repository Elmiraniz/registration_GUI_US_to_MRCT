...
if isreal(v)
    xyv = matlab.internal.math.mergesimpts([y, x, v], [myepsy, myepsx, Inf], 'average');
    x = xyv(:,2);
    y = xyv(:,1);
    v = xyv(:,3);
else
    % if z is imaginary split out the real and imaginary parts
    xyv = matlab.internal.math.mergesimpts([y, x, real(v), imag(v)], ...
...
if(isreal(v))
    vq = matlab.internal.math.cubic(x,y,v,xq,yq,tri,t);
else
    vre = real(v);
    vim = imag(v);
    vqre = matlab.internal.math.cubic(x,y,vre,xq,yq,tri,t);
    vqim = matlab.internal.math.cubic(x,y,vim,xq,yq,tri,t);
